    ...
    inner class ViewHolder(val binding: TasksActivityBinding) :
        ...
        fun bind(item: Activity) {
            ...
            binding.deletebtn.setOnClickListener {
                ...
                // search for activity node by name under current user's activities
...
//            }
            ...
            binding.checkbox.setOnCheckedChangeListener { _, isChecked ->
                ...
                // find the correct child(s) and update fields
                database.child("users").child(userId).child("activities")
                    .orderByChild("name").equalTo(item.name)
                    .get()
                    .addOnSuccessListener { snapshot ->
                        if (snapshot.exists()) {
                            ...
                            val msg = if (isChecked) "Completed ${item.name}" else "Marked ${item.name} pending"
                            Toast.makeText(context, msg, Toast.LENGTH_SHORT).show()
                        } else {
                            Toast.makeText(context, "Activity not found in DB", Toast.LENGTH_SHORT).show()
                        }
                    }
                    .addOnFailureListener {
